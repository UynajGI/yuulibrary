...
title: "第4章 · 不基于模型的预测"
weight: 13
description: "4.1 蒙特卡罗强化学习 · 4.2 时序差分强化学习 · 4.3 n步时序差分学习 · 1.前向认识TD（λ） · 2"
---
# [第4章](ch04.md) 不基于模型的预测
...
$$
\begin{array}{l} \mu_{k} = \frac{1}{k} \sum_{j = 1} ^{k} x_{j} \\ = \frac{1}{k} \left(x_{k} + \sum_{j = 1} ^{k - 1} x_{j}\right) \\ = \frac{1}{k} \left(x_{k} + (k - 1) \mu_{k - 1}\right) \\ = \mu_{k - 1} + \frac{1}{k} \left(x_{k} - \mu_{k - 1}\right) \\ \end{array}
$$
...
蒙特卡罗学习（a=1）
![](images/7a4698d65f2c9f4a703db62855444e36f0f94fff32ed223b8f5b66fe95f5fa09.webp)


状态

时序差分学习（a=1）
![](images/54026b3363f6ff550722f7de2c2a907f52e5f6c7ca3a7fe1e99287617441cb01.webp)


状态

{{< caption >}}图4.1　MC学习和TD学习在驾车返家示例中的比较{{< /caption >}}
...
{{< caption >}}表4.2　A、B状态转移经历{{< /caption >}}
...
$$
\hat{\boldsymbol{P}} _{s, s^{\prime}} ^{a} = \frac{1}{N (s , a)} \sum_{k = 1} ^{K} \sum_{t = 1} ^{T_{k}} 1 \left(S_{t} ^{k}, a_{t} ^{k}, s_{t + 1} ^{k} = s, a, s^{\prime}\right)
$$
...
![](images/70e53aa4b127388c386826d05583f36f30d4d45ed38ec9411367695b1eb29bf5.webp)


{{< caption >}}图4.2　TD算法构建的MDP{{< /caption >}}
...
![](images/4035d58ed43c3450954d5a130b86850d6e71b94e0b735a0844448e0d2a861e8b.webp)


{{< caption >}}图4.3　MC学习深度采样回溯{{< /caption >}}

·TD学习算法：浅层采样学习。一次学习可以经历不完整的状态序列，使用后续状态的预估状态价值来预估收获值，再更新当前状态价值，如图4.4所示。

![](images/cb2a51136f9d16d14eeddec6d6450afd3417da2648f86e753446406afc8a27c2.webp)


{{< caption >}}图4.4　TD学习浅层采样回溯{{< /caption >}}

·DP学习算法：浅层全宽度学习。依据模型，全宽度地使用后续状态预估价值来更新当前状态价值，如图4.5所示。

![](images/8207ab28874138c7e7a16fb702a70a2f6bdc672de14a91d8ef9d9bc44a6106f7.webp)


{{< caption >}}图4.5　DP学习浅层全宽度（采样）回溯{{< /caption >}}
...
![](images/6846f82267bc265bcb7343da43401cde83cb659ff5e41584261ca4f247657028.webp)



{{< caption >}}图4.6　n步预测{{< /caption >}}

TD是TD（0）的简写，是基于1步预测的。根据n步预测的定义，可以推出当n=1、2和∞时对应的预测值如表4.3所示。从该表可以看出，MC学习是基于∞步预测的。

{{< caption >}}表4.3　n步收获{{< /caption >}}
...
TD()，收获
![](images/de3913ecfc473745f5fff28ef6921ee61536a78bab1d6441fe15addc70edbde6.webp)


{{< caption >}}图4.7　λ收获权重分配{{< /caption >}}
...
![](images/1aff2359577819f885b50ed0353c35f8c0ed0b78b4dd40d623268295b304db20.webp)


{{< caption >}}图4.8　TD（λ）对于权重分配的图解{{< /caption >}}
...
![](images/de360ffd5e0f91da8351be008d7d57401a6d689d4ea8ef0be0077fe69ecbc90b.webp)


{{< caption >}}图4.9　是响铃还是亮灯引起了老鼠遭电击{{< /caption >}}
...
![](images/08e17c32a6851c4e96e30c957c3f4003a53a7bffb36e3c45b9a8a81c0ced5555.webp)



{{< caption >}}图4.10　某状态一个可能的效用时间曲线{{< /caption >}}
...
$$
\begin{array}{l} \delta_{t} = R_{t + 1} + \gamma V (S_{t + 1}) - V (S_{t}) \\ V (s) \leftarrow V (s) + \alpha \delta_{t} E_{t} (s) \tag{4.9} \\ \end{array}
$$
...
玩家类也继承自游戏者（Gamer），指定其策略为最原始的策略（naive_policy），规定玩家只要点数小于20点就继续叫牌。玩家同时还会根据当前局面信息得到当前局面的状态，为策略评估做准备。
...
这个方法（serve_card_to）接受一个玩家（player）和一个整数（n）作为参数，表示向该玩家一次发出一定数量的牌，在发牌时如果遇到发牌器里没有牌的情况时，会将已使用的牌收集起来洗好后送入发牌器，随后把需要数量的牌发给某一玩家。代码中的方法（_info）负责根据条件在终端输出对局信息。
...
```python
    def play_game(self, dealer, player):
        ...
        while True:
            ...
            ## 玩家的策略产生一个行为
            self._info("{}选择: {};".format(player.role, action))
            ## 记录一个(s,a)
...
```python
    def play_games(self, dealer, player, num=2, show_statistic = True):
        ...
        if show_statistic:
            print("共玩了 {}局，玩家赢 {}/和 {}/输 {}，胜率：{:.2f}, 不输率：{:.2f}"
...
# 将输出类似如下的结果
# 100%|██████████| 200000/200000 [00:18<00:00, 11014.64it/s]
...
![](images/7ff40e4e991a05258755c1032a31490ea48eb0e602c52e57ea97ae9f22349c09.webp)


（a）有"可用的牌A"

![](images/30a24c909b9017dd04ce9ad43cd854c5780ceda68260d2e03dd3c2d209c7b168.webp)


（b）没有"可用的牌 $A ^{\prime \prime}$
{{< caption >}}图4.11 21点游戏玩家原始策略的价值函数（20万次迭代）{{< /caption >}}
