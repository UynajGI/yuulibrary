...
初始化：参数可微的值函数 $\hat{v}(s,w)$ 。初始参数 $w_0$ 。目标：估计一个给定策略 $\pi$ 的状态值。对于由 $\pi$ 生成的每个回合 $\{(s_t,r_{t + 1},s_{t + 1})\}_{t}$ 对于每个样本 $(s_t,r_{t + 1},s_{t + 1})$ 对于一般值函数： $w_{t + 1} = w_t + \alpha_t[r_{t + 1} + \gamma \hat{v} (s_{t + 1},w_t) - \hat{v} (s_t,w_t)]\nabla_w\hat{v} (s_t,w_t)$ 对于线性值函数： $w_{t + 1} = w_t + \alpha_t[r_{t + 1} + \gamma \phi^{\mathrm{T}}(s_{t + 1})w_t - \phi^{\mathrm{T}}(s_t)w_t]\phi (s_t)$
