...
最后值得注意的是， $\mathbb{E}[X|A = a]$ 与 $\mathbb{E}[X|A]$ 不同。前者是一个值，而后者是一个随机变量。实际上， $\mathbb{E}[X|A]$ 是随机变量 $A$ 的函数，此时需要用更严格的概率论来定义 $\mathbb{E}[X|A]$ ，这会在[附录B](#附录b)中讨论。 <!-- validate-skip -->
