...
如果 $q_{t}(s_{t},a_{t})$ 是通过蒙特卡罗方法来估计的，那么相应的算法被称为REINFORCE或者蒙特卡罗策略梯度。该算法已经在第9章介绍过了。
...
算法 10.1：最简单的 Actor-Critic 算法（QAC）
...
<div class="mineru-algorithm" style="white-space: pre-wrap; font-family:monospace;">
算法10.3：基于重要性采样的Off-policy Actor-Critic算法
初始化：给定一个行为策略 $\beta (a|s)$ 。一个目标策略 $\pi (a|s,\theta_0)$ ，其中 $\theta_0$ 是初始参数。一个值函数 $v(s,w_0)$ ，其中 $w_{0}$ 是初始参数。 $\alpha_w,\alpha_\theta &gt;0$ 。目标：学习一个最优策略以最大化 $J(\theta)$ 。
在每个回合中的 $t$ 时刻按照 $\beta (s_t)$ 生成 $a_{t}$ ，然后得到 $r_{t + 1},s_{t + 1}$ 优势函数（时序差分误差）：$\delta_t = r_{t + 1} + \gamma v(s_{t + 1},w_t) - v(s_t,w_t)$ Actor（策略更新）：$\theta_{t + 1} = \theta_t + \alpha_\theta \frac{\pi(a_t|s_t,\theta_t)}{\beta(a_t|s_t)}\delta_t\nabla_\theta \ln \pi (a_t|s_t,\theta_t)$ Critic（值更新）：$w_{t + 1} = w_t + \alpha_w\frac{\pi(a_t|s_t,\theta_t)}{\beta(a_t|s_t)}\delta_t\nabla_wv(s_t,w_t)$
