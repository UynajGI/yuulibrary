...
instability, concentration, and underperformance. HRP 应用现代数学（图论和机器学习技术），基于协方差矩阵中包含的信息构建分散化投资组合。 However, unlike quadratic optimizers, HRP does not
require the invertibility of the covariance matrix. 事实上，HRP 可以在病态退化甚至奇异协方差矩阵上计算投资组合，这对二次优化器来说是不可能的壮举。 Monte Carlo
...
more than six decades ago. 他的不朽洞见是认识到不同风险水平对应不同的风险调整收益最优投资组合，因此有了「有效前沿」的概念 (Markowitz [1952]). 一个含义是，将所有资产分配给期望收益最高的投资很少是最优的。 Instead, we should take into account the correlations across
...
unstable solutions. 分散化的好处往往被估计误差所抵消甚至超过。
...
minimum eigenvalue is lower. 条件数 rises quickly, leading
...
gain an intuition of how HRP 工作. Scipy\'s
