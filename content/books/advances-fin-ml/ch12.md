...
两种方式：(1) 狭义上，模拟投资策略的历史表现，就好像它过去运行过一样；(2) 广义上，模拟过去未发生的情景。第一种（狭义）方法也称为走步法（walk-forward），如此流行，以至于事实上「回测」已成为] *de
facto* [「历史模拟」的同义词。第二种（广义）方法远不那么为人所知，在本章中，我们将介绍一些执行它的新方法。 Each approach has its pros and cons, and
each should be given careful consideration.

## 12.2 走步法
文献中最常见的回测方法是走步法（Walk-Forward, WF）。WF 是对策略过去表现的模拟。 每个策略决策都基于先于该决策的观测。 正如我们在[第 11 章](ch11.md)中所见，执行无瑕疵的 WF 模拟是一项艰巨的任务，需要对数据源、市场微观结构、风险管理、绩效衡量标准（如 GIPS）、多重检验方法、实验数学等有深入了解。 Unfortunately, there is no
...
WF 有两个关键优势： (1) WF has a clear historical
...
WF 有三个主要缺点： First, a single scenario is
...
## 12.3 交叉验证法
投资者经常问，如果策略遭受像 2008 年危机那样不可预见的压力情景，它会表现如何。 或互联网泡沫、缩减恐慌、2015-2016 年中国恐慌等。 One
...
通过交叉验证（CV）回测的目标不是推导历史准确的性能，而是从多个样本外情景中推断未来性能。 For each period of the backtest, we
...
## 12.4 组合净化交叉验证法
在本节中，我将介绍一种新方法，解决 WF 和 CV 方法的主要缺点，即这些方案测试单一路径。 I call it the "combinatorial purged cross-validation"
