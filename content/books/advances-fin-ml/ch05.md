...
分数阶差分的概念应用于预测性时间序列分析至少可以追溯到 Hosking [1981]。 In that paper,
...
让我们讨论如何在实践中对（有限）时间序列进行分数阶差分。 Suppose a time series with] *T* [real
observations,  *X ~[*t*]~*
}, ] *t* [= 1, ...,] *T* [.
...
两个图中的负漂移是由窗口扩展时添加到初始观测的负权重引起的。 When we
...
或者，可以使用固定宽度窗口计算分数阶差分，即在权重的模之后丢弃权重
