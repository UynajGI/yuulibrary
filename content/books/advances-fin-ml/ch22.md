...
real-time or near-real-time responses. 例如，电网中的不稳定事件可能在几分钟内发展成灾难；足够快地找到预兆特征可以避免灾难。 Similarly, signs of emerging illiquidity events have been
...
数据流按定义是渐进可用的；因此，可能没有大量数据对象可以并行处理。 Typically, only a fixed amount of the most recent data records
...
time-step) to many CPU cores. 对于此类工作，HPC 生态系统比云生态系统拥有更先进的工具。
...
凭借一些早期支持者的慷慨捐赠，我们建立了一个专门用于此项工作的相当大的计算集群。 This dedicated computer (named dirac1) allows users to utilize an
HPC system and evaluate their applications for
themselves.

我们还参与了使 HPC 系统更适用于流数据分析的工具开发工作。 In the following sections, we
...
网络系统由两部分组成：连接集群内组件的 InfiniBand 网络，以及与外部世界的交换网络连接。 In this particular example, the outside
connections are labeled "ESNet" and "ANI." The InfiniBand network
switches are also common in cloud computing systems.

图 1 中的存储系统包括旋转磁盘和闪存。 This combination is also common. What is different is that a
...
虚拟化通常在云计算环境中使用，以使同一硬件可供多个用户使用，并将一个软件环境与另一个隔离。 This is one of the more prominent
...
migrated to another node. 然而，这个虚拟化层也施加了一些运行时开销，可能降低应用性能。
...
MPI 的成功部分归因于它将语言无关规范（LIS）与语言绑定分离。 This allows
