...
一组高能物理学家认为他们的用例非常适合云计算，并进行了一项详细的比较研究 (Holzman et al. [2017]). 他们的成本比较仍然显示商业云产品对于可比计算任务比专用 HPC 系统贵约 50%；然而，作者在数据入口和出口方面受到严格限制，以避免数据移动可能产生的高昂费用。对于本书讨论的流数据分析等复杂工作负载，我们预计这种 HPC 成本优势将在未来保持。2016 年国家科学院研究得出相同结论，即即使是亚马逊的长期租赁也可能比 HPC 系统贵 2 到 3 倍，以处理 NSF 预期的科学工作负载（National Academies of Sciences [2016]，Box 6.2）。

## 22.5 HPC 软件
具有讽刺意味的是，超级计算机的真正力量在于其专用软件。 HPC 系统和云系统都有各种各样的软件包可用。 在大多数情况下，同一软件包在两个平台上都可用。因此，我们选择专注于 HPC 系统独有的、有潜力改进计算智能和预测技术的软件包。
...
软件工具的讨论将从 MPI 开始。由于本书依赖于数据处理算法，我们将专注于数据管理工具（Shoshami 和 Rotem [2010]）。

### 22.5.1 Message Passing Interface

消息传递接口是一种用于并行计算的通信协议 (Gropp, Lusk, and Skjellum [1999], Snir et al. [1988]). 它定义了多种点对点数据交换操作以及一些集合通信操作。MPI 标准是在几次早期构建可移植通信库的尝试基础上建立的。来自 Argonne National
...
MPICH 使用的许可证。该许可证允许供应商获取源代码来产生自己的定制版本，使 HPC 系统供应商能够快速产生自己的 MPI 库。时至今日，所有 HPC 系统都在其计算机上支持熟悉的 MPI。这种广泛采用也确保了 MPI 将继续是 HPC 系统用户中最受欢迎的通信协议。
...
HDF
Group。^2^
信息（如属性、维度和数据类型）。它们合在一起称为数据集。数据集可以分组为更大的单元（称为组），组可以组织为更高级别的组。这种灵活的层次组织允许用户表达数据集之间的复杂关系。

除了将用户数据组织到文件中的基本库之外，HDF Group 还提供了一套工具 and specialization of HDF5 for
不同的应用。例如，HDF5 包含一个性能分析工具。NASA 有一个 HDF5 的专业化，名为 HDF5-EOS，用于来自其地球观测系统（EOS）的数据；下一代
DNA 序列社区产生了一个名为 BioHDF 的专业化，用于
their bioinformatics data.

HDF5 提供了访问 HPC 上存储系统的有效方式
平台。在测试中，我们展示了使用 HDF5 存储股票市场数据显著加速了分析操作。这主要归功于其高效的压缩/解压缩算法，最小化了网络流量和 I/O 操作，这引出了我们的下一个要点。
...
在当前一代处理系统中，自适应 I/O 系统（ADIOS）是使用最广泛的 (Liu et al. [2014]). 它使用多种数据传输引擎，允许用户接入 I/O 流并执行分析操作。 这很有用，因为无关数据可以在传输过程中丢弃，从而避免其缓慢和大量的存储。 This same] *in situ* [机制也使其能够非常快速地完成写操作。事实上，它最初因其写入速度而引起关注。此后，
...
尽管收敛似乎是不可避免的，我们倡导保持上述软件工具优势的收敛选项。 CIFT 项目的动机之一是寻找一种将上述工具转移到未来计算环境的方法。

## 22.6 用例
数据处理是现代科学研究如此重要的一部分，以至于一些研究者称之为科学的第四范式
（Hey、Tansley 和 Tolle [2009]）。在经济学中，同样的数据驱动研究活动催生了广受欢迎的行为经济学（Camerer 和 Loewenstein [2011]）。 最近数据驱动研究的许多进展基于机器学习应用 (Qiu
...
识别和分类任务过去由天文学家手动执行。 然而，目前 PTF 望远镜传入的图像数量太大，无法人工检查。 An automated
用于这些图像处理任务的自动化工作流已被开发并部署在多个不同的计算中心。
...
该超新星的快速识别是自动化工作流程机器学习能力的重要证明。 该工作流处理传入图像以提取自上次观测以来发生变化的对象。 然后基于之前的训练对变化对象进行分类以确定初步类型。 由于从快速变化的瞬变体中提取新科学的后续资源非常宝贵，分类不仅需要指示假设类型，还需要指示分类的可能性和置信度。 使用在 PTF 数据上训练的分类算法，瞬变体和变星的错误标记总错误率为 3.8%。 预计额外工作将在即将到来的巡天中实现更高的准确率，如大型巡天
望远镜。
