...
避免与训练集相关的示例泄露到测试集中。接下来，我们将介绍一种用于策略选择的 CV 方法，
...
我们施加的唯一条件是 (1) M 是一个
真正的矩阵，即每列具有相同的行数，其中观测在所有
the*N*次试验的每一行上是同步的，且 (2) 用于选择「最优」策略的绩效评估指标可以在每列的子样本上估计。 例如，如果该指标是夏普比率，我们假设 IID 正态分布假设可以在报告表现的各种切片上成立。 如果不同模型配置以不同频率交易，观测被聚合（下采样）以匹配共同索引] *t*
= 1, ...,*T*.

第二，我们将 M 按行划分为偶数 *S* [个相等维度的不相交子矩阵。每个子矩阵] *M
~[*s*]~*, with*s*= 1,
...,*S*, is of order
...
1.  形成*训练集 J*，通过连接
    ...
    子矩阵 *M~s~*（构成 *c*）。*J* 是一个矩阵
    of order ![](images/Image00239.webp)
    .
2.  形成*测试集*，作为 *J* 在 *M* 中的补集。换
    言之，
...
{{< caption >}}图 11.1 Best Sharpe ratio{{< /caption >}}
in-sample (SR IS) vs Sharpe ratio out-of-sample (SR OOS)

![](images/Image00304.webp)

{{< caption >}}图 11.2 Probability of{{< /caption >}}
