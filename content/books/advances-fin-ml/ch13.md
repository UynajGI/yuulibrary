...
elements in Ω is large. 交易规则引入了这样的自由变量，
...
## 13.4 我们的框架
到目前为止，我们还没有表征观测 π ~[*i*\ ,\ *t*]~ are drawn. 我们感兴趣的是为过拟合最具破坏性的场景找到最优交易规则（OTR）， such as when π
...
根据 Bailey 和 López de Prado [2013] 中命题 4 的证明，
...
-   **Step 1** : 我们估计输入参数 {σ, φ}, by linearizing
    equation ( 13.2 ) as:
...
-   **Step 2** : 我们构建止损和止盈的网格
    ...
    Ω.
-   **Step 3** : 我们生成大量路径 (e.g., 100,000) for
    ...
    3).^\ [3]^
-   **Step 4** : 我们将 100000 条路径应用于 generated in Step 3 on each
...
## 13.6 实验结果
表 13.1 [列出了本研究中分析的组合。 虽然这些输入参数的不同值会产生不同的数值结果， the combinations
...
在以下图中，我们绘制了非年化夏普比率 that result from various combinations of profit-taking and
...
零长期均衡的情况与做市商的业务一致， who provide liquidity under the assumption that price
...
通过历史模拟在随机游走上校准交易规则将导致回测过拟合， because one random
...
正长期均衡的情况与持仓者（如对冲基金或资产管理公司）的业务一致， such as a hedge-fund or asset
manager.]  Figure
