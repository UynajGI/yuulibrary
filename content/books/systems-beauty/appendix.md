...
下列这些公式是用于[第1章](ch01.md)和[第2章](ch02.md)中讨论的那9个动态模型的。“转化器”（Converters）可以是常数，或是基于系统模型中的其他要素得出的计算结果。（t）是时间的缩写，（dt）代表时间间隔，即从本次计算到下一次计算之间的时
...
咖啡的温度= $100^\circ\mathrm{C}$、$80^\circ\mathrm{C}$、$60^\circ\mathrm{C}$ ……分别对应三个对比模型
...
初始存量值：咖啡的温度= $0^\circ\mathrm{C}$、$5^\circ\mathrm{C}$、$10^\circ\mathrm{C}$ ……分别对应三个对比模型
...
本书由我进行翻译，北京学而管理咨询有限公司的崔玲女士协助翻译了[第1章](ch01.md)和附录部分，并对全书进行审核、校对，最后由我进行全文审校、定稿。
