...
将 Wick 定理的结果代入 $Z$ 的展开式。原始展开系数的倒数因子为 $1/(2n)!$。

**第一步：选择哪些位置放产生算符。** 从 $2n$ 个相互作用顶点中选 $n$ 个为产生算符（余下 $n$ 个为湮灭算符），组合数为 $\binom{2n}{n}$：
...
**第二步：排列求和。** 代入 Wick 收缩后，对湮灭-产生的配对排列求和。定义 $\tau_k' = \tau_{n+k}$，暂时得到

$$
\frac{1}{n!^2} \sum_{\pi \in S_n} (\cdots)_{\pi}.
$$

**第三步：哑变量重标号消除排列求和。** $\tau_1', \ldots, \tau_n'$ 都是积分哑变量。对每个排列 $\pi$，通过重标号 $\tau_k' \to \tau_{\pi[k]}'$，积分值完全相同。因此 $\sum_{\pi} (\cdots)_{\pi} = n! \, (\cdots)_{\mathrm{id}}$（恒等排列），

$$
\frac{1}{n!^2} \cdot n! \, (\cdots)_{\mathrm{id}} = \frac{1}{n!} \, (\cdots)_{\mathrm{id}}.
$$

三个因子：$\binom{2n}{n}$ 来自选位置，$\sum_{\pi \in S_n}$ 来自 Wick，重标号消去 $\sum_{\pi}$ 产生 $n!$ —— 净效果是 $1/n!$。
