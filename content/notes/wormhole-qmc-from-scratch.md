...
本节的定位：下面这张表是整条推导路线的全局地图，标注了每一章在链路中的角色。

| 阶段 | 对应章节 | 核心动作 | 输出结果 |
|------|----------|----------|----------|
| 模型定义 | §2 | 写出自旋-玻色哈密顿量，拆分自旋/浴/耦合 | 通用哈密顿量形式 |
| 解析约化 | §3--4 | Dyson 展开 + Wick 定理，精确积分掉所有玻色自由度 | 纯自旋的推迟作用量 $\hat{\mathcal{H}}_{\mathrm{ret}}$ |
| 采样方案设计 | §5 | 级数展开，拆解为「连续分布 × 离散自旋顶点」 | 三因子分解、顶点算符 |
| 构型空间具象化 | §6 | 在 $S_z$ 基下算矩阵元，得到具体顶点类型和数值权重 | 六种四腿顶点、权重 $W_v$ |
| 采样算法实现 | §7--8 | 设计两类 MC 更新，分别采样顶点数量和世界线构型 | 对角更新、wormhole 更新 |
| 后处理与边界 | §9--10 | 可观测量测量、算法适用条件 | 物理量计算、无符号条件 |
...
**为什么叫"虫洞"？** 和格点 SSE 类比就很清楚：
- 格点海森堡模型：非对角顶点是空间两格点算符 $\hat{S}_i^+ \hat{S}_j^-$，loop 在格点 $i$ 和 $j$ 之间跳跃——是**空间跳跃**
- 自旋-玻色模型：非对角顶点是虚时间两时刻算符 $\hat{S}^+(\tau) \hat{S}^-(\tau')$，loop 在虚时间 $\tau$ 和 $\tau'$ 之间跳跃——相当于在虚时间轴上打了一个洞，世界线瞬间从一个时刻跳到另一个时刻，所以叫**虫洞（wormhole）**

**拓扑等价性——这是整个算法最核心的数学事实。** 推迟顶点的四腿结构与格点模型中最近邻交换顶点的四腿结构数学上完全同构。区别仅是 loop 沿空间键跳还是沿虚时间虫洞跳。正因此，标准 directed-loop 算法的细致平衡条件、概率方程、bounce 优化全部可以原封不动搬过来——§8.1--§8.4 就是对这些已有算法的系统复述。
...
| 基本单元：空间键 $(i,j)$ | 基本单元：虚时间对 $(\tau,\tau')$ | 都是两体相互作用的载体 |
| 展开项：键算符顶点 | 展开项：推迟顶点 | 都是级数展开的基本单元 |
| 非对角顶点：$\hat{S}_i^+\hat{S}_j^-$ | 非对角顶点：$\hat{S}^+(\tau)\hat{S}^-(\tau')$ | 产生一对自旋翻转 |
| loop 跳跃：跨空间格点 | loop 跳跃：跨虚时间点（虫洞） | 传播维度不同，数学结构一致 |
| 对角更新：增删对角键算符 | 对角更新：增删对角推迟顶点 | 调节展开阶数 |
| 世界线：自旋沿虚时间的演化 | 世界线：自旋沿虚时间的演化 | 同一概念 |
