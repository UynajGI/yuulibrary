...
> **Phase 4.2 已完成的，本阶段不重复**：翻译（translate_chapters.py）、引用/图注/来源的 callout/caption 转换（翻译脚本内置）、交叉引用链接（convert_xrefs.py）。本阶段聚焦脚本无法覆盖的语义检查。
...
**🔴 `title` 必须是中文**——英文书名翻译为中文，原文放 `book-cover-subtitle`。

```markdown
---
title: "<书名>"            # 🔴 必须是中文！书架菜单显示中文标题
description: "<一句话简介>"
...
| display-math 块内有中文正文（validate E） | LLM 输出相邻 inline-math 被误判为 display-math → fix_math_delimiters 自动降级 | 手动把误包的 display-math 改为 inline-math |
...
| 37 | `_index.md` 的 `title` 写英文 | **必须翻译成中文**——书架菜单直接显示 `title`，英文书名放 `book-cover-subtitle` |
