...
├── _index.md                  # 翻译正文 + SM 附录 + 阅读笔记（全在一篇里）
...
**🔴 关键规则（架构约定，执行细节见各 Phase）**：
- 论文目录用 `_index.md`（section 列表页），**不是**普通 `.md`
- **🔴 slug 用 `firstAuthorYear-description` 格式**：如 `boulebnane2026-spin-boson-qaoa`，不用纯描述性 slug
- front matter 字段必须齐全；`weight` 比已有最后一篇 +1
- **🔴 LaTeX 公式 100% 原样**，图注用 `{{< caption >}}`，强调用 `{{< callout >}}`
- **🔴 SM 有就必须和正文一起处理**：两份都提取、都翻译、都合并（SM 插在正文参考文献之后、`## 阅读笔记` 之前）。没有 SM 则跳过
- **不更新首页书架**：论文在 `/papers/` section 自动列出
- **不在论文里手写目录**：直接用 `##`/`###` 分节
...
date: 2026-07-09T08:00:00+08:00            # 🔴 带时区！见下方「日期陷阱」
author: "<作者列表，逗号分隔>"
year: 2026
...
🔴 **CHECKPOINT**：确认去重通过（0 结果）再继续。不要跳过。
...
🔴 **CHECKPOINT**：确认提取完成——MD 文件非空、`images/` 非空、行数 > 10。如果有 SM PDF，确认两份都提取了。
...
6. **validator 过**（`validate_book.py` 对 paper 也能跑基本检查：`$$` 配对、裸代码、naked caption 等）：
   ...
   7. **🔴 提交前预检**（避免反复 commit-fix-commit）：
      ```bash
      # 跑 lefthook 会触发的三项检查，0 error 再 git commit
      markdownlint content/papers/<slug>/_index.md
      python3 scripts/check_latex_render.py
      # images 检查由 lefthook image-refs hook 覆盖，提交时自动跑
      ```
...
**修复（推荐方案 1，最可靠）**：
1. **带时区写当天**（推荐）：`date: 2026-07-09T08:00:00+08:00`。Hugo 正确解析时区后判断过去/未来，不会误判。用北京时间（UTC+8）当天上午，永远安全。
2. **写昨天的日期**（备选）：`date: 2026-06-27`。论文笔记用「加入日期」而非「写作时刻」，差一天无所谓。但架子上日期就少一天——不推荐。
