...
│   └── _partials/docs/inject/head.html  # 主题切换无闪烁脚本（顶部）+ KaTeX + pseudocode.js 加载
├── assets/custom.scss            # 全局样式
├── static/
│   ├── katex/ + pseudocode/ + rough.min.js  # 本地化数学/算法/手绘渲染
│   ├── chat/chat.js + chat.css              # AI 问答 Agent（BYOK 浏览器直连）
│   └── pageindex/                           # PageIndex 树索引 JSON（进 git，含 summary + source_md + line_num，无 text）
...
├── .github/workflows/deploy.yml  # 打 tag（YYYY.MM.DD.NN）→ 自动部署；push main 不触发
├── scripts/release.sh             # 算下一个发布 tag（只读，手动确认后打）
...
- **3 个工具**：`search_library`（BM25+精排+MMR 检索）、`get_section`（按 doc_id+node_id 取完整章节，从 GitHub raw fetch md 按行号切）、`rewrite_query`（LLM 改写/分解/步退查询）。system prompt 注入全局目录（55 篇文档 TOC）+ 查询转换策略引导
- **LLM 重排**：confidence=low 时批量评分重排 top6（一次 API 调用，无 key 跳过）
- **ReAct agent loop**：模型自主调用工具检索，多轮推理（最多 4 轮工具调用），不靠单次 RAG
...
- **CI 部署**：deploy.yml 只在打 tag（`YYYY.MM.DD.NN`）时触发，pageindex 在 deploy 时增量重建（不 commit 回 main，只部署到 gh-pages）。平时 push 到 main 不跑 CI。打 tag 用 `scripts/release.sh` 算下一个值，手动确认后 push
...
- **lefthook pre-commit**：trailing-whitespace → prettier → eslint → css-check → hugo-build-check → pageindex-build → markdownlint → image-refs → front-matter → book-validate → paper-validate → latex-render
...
- **`latex-render`**（lefthook pre-commit，独立脚本 `scripts/check_latex_render.py`）：抓两种常见 Markdown/KaTeX 渲染坑——表格 LaTeX 里的裸 `|`（被当列分隔，用 `\lvert`/`\rvert`/`\vert`）和 `$$` 块内行首 `+`/`-`（被当列表项，用 `\;+\;` 或单行）
- **`test_translate.py`**（CI 步骤，34 用例零依赖）：测 `translate_chapters.py` 的纯函数（isolate_references/split_into_chunks/restore_images/find_untranslated_blocks/is_chinese_text），拦"改函数忘改调用方"类 bug。跑：`python3 .claude/skills/add-book-to-library/scripts/test_translate.py`
...
| **check_latex_render.py** | 脚本 | lefthook latex-render hook 的实现（表格裸 `\|` + display math 行首 `+`/`-` 检测） |
| **release.sh** | 脚本 | 算下一个发布 tag（`YYYY.MM.DD.NN`），只读，手动确认后打 |
...
- **馆藏统计**：`{{< stats >}}` 首页仪表盘——自动统计 books/papers/notes 数量 + 最近添加 6 条 + 标签云（按 `Site.Taxonomies.tags` 频次，最多 30 个）。无参数，加到 `_index.md` 即可
...
- 表格：全宽，索引页 6 栏
- 图片：居中带阴影
- **主题切换（日/夜/自动三态）**：左侧菜单底部按钮，localStorage 持久化。架构是 `data-theme`（偏好 light/dark/auto）+ `data-effective-theme`（实际生效 light/dark，CSS 只看这个）。`hugo.toml` 的 `BookTheme='light'`（主题只编译浅色，深色由运行时接管），所有 dark 样式用 `:root[data-effective-theme="dark"]` 选择器（不再用 `@media prefers-color-scheme`）。无闪烁脚本在 `head.html` 顶部同步执行；切换逻辑在 `static/js/theme-toggle.js`
