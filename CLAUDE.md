...
├── .claude/skills/add-book-to-library/   # 加书 skill（translate_chapters.py / convert_xrefs.py / validate_book.py）
├── .claude/skills/add-paper-to-library/  # 加论文 skill（clean_paper_md.py / generate_paper_note.py）
...
8. Phase 4.5 审核脚本未覆盖的语义问题（元素模板/OCR/Mermaid/标题层级/伪代码）→ `validate_book.py` 验证
...
5. **翻译 + 结构化分析用 workflow 脚本**（不召唤 subagent）：
   - `clean_paper_md.py` — 清洗 MinerU 噪声（出版元数据/目录/脚注）+ 修 LaTeX 碎片 + 图注转 caption + 标题层级修复
...
| **translate_chapters.py** | 脚本 | 翻译英文 MD → 中文（种子章建术语表 + 并发 + validate 验证重试）。book 原地写，paper 输出 `.zh.md` |
| **convert_xrefs.py** | 脚本 | 纯 regex 交叉引用转换（第N章→[第N章](ch0N.md)，补零可靠） |
| **clean_paper_md.py** | 脚本 | 论文 MD 清洗：噪声删除 + LaTeX 碎片修复 + 图注配对 + 标题层级 |
