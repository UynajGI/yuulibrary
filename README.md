...
共 21 本，按主题分类（与首页书架一致）。书籍和论文笔记的书架卡片均为自动生成——新增内容只需填好 `_index.md` 的 front matter（`tags`/`author`/`description`），无需手动编辑 shortcode。
...
共 24 篇，按主题分类（几何相位与几何量子计算 / 电路 QED / 线性响应理论 / 量子光学与层析 / 机械振子量子化），详见 `/papers/` 页面。
