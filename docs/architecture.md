...
| 文件 | 行数 | 内容 |
|------|------|------|
| `static/chat/chat.js` | ~2050 | 完整 Chat Agent：BM25 检索 + ReAct 工具调用 + 多 provider SSE 流式 + 会话管理 + UI 渲染。零依赖 vanilla JS |
| `static/chat/chat.css` | ~840 | 聊天面板样式：浮动按钮、侧边抽屉、消息气泡、思考折叠、代码块、响应式 |
| `layouts/_partials/docs/inject/head.html` | — | 注入 `window.YUU_CHAT_RAW_BASE`(从 `BookRepo` 推导 raw URL) + 加载 chat.js/css |
