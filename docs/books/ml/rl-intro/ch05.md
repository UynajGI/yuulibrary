...
<p class="caption">图5.1　Sarsa 算法示意图</p>
...
state 输出: Q
state initialize: set Q(s, a) arbitrarily, for each s in S and a in A(s); set Q(terminal state, ·) = 0
repeat
    state initialize: S ← first state of episode
    state A = policy(Q, S) (e.g. ε-greedy policy)
    repeat
...
在Sarsa算法中，Q（S,A）的值使用一张大表来存储，这不是很适合解决规模很大的问题；对于每一个状态序列，在S状态时采取的行为A是基于当前行为策略的，也就是该行为是与环境进行交互实际使用的行为。在更新“状态-行为对”（S,A）的价值的循环里，个体状态$\mathrm{S}'$ 下也依据该行为策略产生了一个行为 $\mathrm{\Delta}\mathrm{\times}$ ，该行为在当前循环周期内用来得到“状态-行为对”（S',A'）的价值，并借此来更新“状态-行为对”（S,A）的价值，在下一个循环周期（时间步）内，状态 $\mathrm{S}'$ 和行为A'将转换身份为当前状态和当前行为，该行为将被执行。
...
<p class="caption">图5.2　有风的格子世界环境</p>
...
<p class="caption">图5.3　有风的格子世界最优路径和Sarsa算法学习曲线</p>
...
类似于TD（λ），可以给n步Q收获中的每一步收获分配一个权重，并按权重对每一步Q收获求和，那么将得到 ${\bigl |} {\mathrm{q}} ^{\lambda}$ 收获，它结合了所有n步Q收获：
...
<pre class="pseudocode">
\begin{algorithm}
\caption{算法 2: Sarsa(λ)算法}
\begin{algorithmic}
state 输入: episodes, α, γ
state 输出: Q
state initialize: set Q(s, a) arbitrarily, for each s in S and a in A(s); set Q(terminal state, ·) = 0
repeat
    state E(s, a) = 0 for each s in S and a in A(s)
    state initialize:
    ...
    repeat
        state R, S' = perform_action(S, A)
        state A' = policy(Q, S') (e.g. ε-greedy policy)
        state δ ← R + γQ(S', A') - Q(S, A)
        state E(S, A) ← E(S, A) + 1
        for{all s ∈ S, a ∈ A(s) do}
            state | Q(S, A) ← Q(S, A) + αδE(s, a)
            state | E(s, a) ← γλE(s, a)
        endfor
        state S ← S'; A ← A'
    until{S is terminal state}
...
Sarsa（λ）算法：该算法同时还针对每一次状态序列维护一个关于“状态-行为对”（S,A）的E表，初始时E表值均为0。当个体首次在起点 $\mathrm{S}_{0}$ 决定移动一步 $\mathrm{\ddot{A}_{0}}$ （假设向右）时，它被环境告知新位置为 $\mathrm{\Delta S_{1}}$ ，此时发生如下事情：首先，个体会做一个标记，使E $(\mathrm{S}_{0},\mathrm{A}_{0})$ ）的值增加

1，表明个体刚刚经历过这个事件 $(\mathrm{S}_{0},\mathrm{A}_{0})$ ）；其次，它要估计这个事件对于解决整个问题的价值，也就是估计TD误差，依据公式可得到结果为0，说明个体认为在起点处向右走没有什么价值。“没有什么价值”有两层含义：一是说明在 $\mathrm{S}_{0}$ 处往右对解决问题没有积极的帮助，二是表明个体认为所有能够到达 $\mathrm{S}_{0}$ 状态的“状态-行为对”的价值没有任何积极或消极的变化。随后，个体将要更新该状态序列中所有已经经历的Q（S,A）值，由于存在E值，那些在 $(\mathrm{S}_{0},\mathrm{A}_{0})$ ）之前近期发生或频繁发生的（S,A）的Q值将改变得比其他Q值明显些，此外个体还要更新其E值，以备下次使用。对于刚从起点出发的个体，这次更新没有使得任何Q值发生变化，仅仅在E $(\mathrm{S}_{0},\mathrm{A}_{0})$ ）处有了一个实质的变化。随后的过程类似，个体的发现就是对路径有一个记忆，体现在E里，具体的Q值没有发生变化。这个情况直到个体到达终点位置时发生改变。此时个体得到了一个即时奖励1，它会发现这一次变化 $( \mathcal{M} \mathrm{S_{H}} )$ 采取向上移步行为 $\mathrm{\Delta A_{up}}$ 到达 $\mathrm{S_{G}}$ ）价值明显，它会计算这个TD误差为1，同时告诉整个经历过程中所有的（S,A），根据它与 $(\mathrm{S_{H}},\mathrm{A_{up}})$ ）的密切关系更新这些“状态-行为对”的价值Q，个体在这个状态序列中经历的所有“状态-行为对”的Q值都将得到一个非0的更新，但是那些在个体到达 $\mathrm{S_{H}}$ 之前就近发生以及频繁发生的“状态-行为对”的价值提升得更加明显。
...
对于上式，我们可以这样理解：个体处在状态 $\mathrm{S_{t}}$ 中，基于行为策略μ产生了一个行为 $\mathrm{A_{t}}$ ，执行该行为后进入新的状态 $S_{\mathrm{t+1}}$ ，异策略学习要做的事情就是，比较异策略和行为策略在状态 $\mathrm{S_{t}}$ 下产生同样的行为 $\mathrm{\Delta A_{t}}$ 的概率的比值，如果这个比值接近1，说明两个策略在状态 $\mathrm{S_{t}}$ 下采取的行为 $\mathrm{R_{t}}$ 的概率差不多，此次对于状态 $\mathrm{S_{t}}$ 价值的更新同时得到两个策略的支持。如果这一概率比值很小，则表明异策略π在状态 $\mathrm{S_{t}}$ 下选择 $\mathrm{A_{t}}$ 的机会要小一些，此时为了从异策略学习，我们认为这一步状态价值的更新不是很符合异策略，因而在更新时打些折扣。类似地，如果这个概率比值大于1，说明按照异策略，选择行为 $\mathrm{\Delta A_{t}}$ 的概率要大于当前行为策略产生 $\mathrm{A_{t}}$ 的概率，此时对该状态的价值更新就可以大胆些。

异策略TD学习中一个典型的行为策略μ是基于行为价值函数$\mathsf{Q(s,a)}$ ）的ϵ贪婪策略，异策略π则是基于 $\mathbf{Q(S,a)}$ ）的完全贪婪策略，这种学习方法称为Q学习（Q Learning）。

Q学习的目标是得到最优价值 $\mathsf{Q(s,a)}$ ），在Q学习的过程中，t时刻与环境进行实际交互的行为 $\mathrm{A}_{\mathrm{t}}$ 由策略μ产生：
...
其中，策略π是一个完全贪婪策略。 $\mathrm{Q(S_{t},A_{t})}$ ）按下式更新：
...
其中，TD目标 $R_{t + 1} + \gamma Q ( S_{t + 1} , A^{\prime} )$ 是基于异策略π产生的行为 $\mathrm{\Delta}\mathrm{\times}$ 得到的Q值。根据这种价值更新的方式，状态 $\mathrm{S}_{\mathrm{t}}$ 依据ϵ贪婪策略得到的行为 $\mathrm{A}_{\mathrm{t}}$ 的价值，将朝着 $\mathrm{S}_{\mathrm{t}+1}$ 状态下贪婪策略确定的最大行为价值的方向做一定比例的更新。这种算法能够使个体的行为策略μ更加接近贪婪策略，同时保证个体能持续探索并经历足够丰富的新状态，并最终收敛至最优策略和最优行为价值函数。

![](images/29f9fbd66a78d07e624a45f41b84fc98e973a2dfe7c10c3513a97c7c608f1943.jpg)


<p class="caption">图5.5　Q学习算法示意图</p>
...
repeat
    ...
    repeat
        state A = policy(Q, S) (e.g. ε-greedy policy)
        state R, S' = perform_action(S, A)
        state Q(S, A) ← Q(S, A) + α (R + γ max_a Q(S', a) - Q(S, A))
        state S ← S'
    until{S is terminal state}
until{all episodes are visited}
\end{algorithmic}
...
<p class="caption">图5.7　21点游戏蒙特卡罗控制学习结果（20万次迭代）</p>
...
class Transition(object):
    def__init__(self, s0, a0, reward: float, is_done: bool, s1):
    self.data = [s0, a0, reward, is_done, s1]
    # ...

class Episode(object):
    def__init__(self, e_id: int = 0) -> None:
...
class Experience(object):
    def__init__(self, capacity:int = 20000):
```
...
```python
def__init__(self,env:Env=None, capacity = 10000):
...
class SarsaAgent(Agent):
    def__init__(self, env:Env, capacity:int = 20000):
...
if display: print(self.experience.last_episode) return time_in_episode, total_reward
